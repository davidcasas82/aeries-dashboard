...
- **Confirm the mark and the Classroom turn-in.** Run `.cursor/skills/verify-aeries-dashboard/verify.sh drive visible --text "Optional Warmup"` and `.cursor/skills/verify-aeries-dashboard/verify.sh drive visible --text "Exit Ticket"`. `Optional Warmup` is under Marked. `Exit Ticket` is under Turned in. Neither is a past-due row.
