...
**v3.1 (Sept 20, 2026), one more paste of `Code.gs` only.** The first v3
...
`{ alt: 'media' }` as the third argument. Fixed in `exportText_`; no
manifest change, no new permission prompt, trigger unchanged. This is a
script bug, not a product change, so it falls under principle 6's
exception. `UrlFetchApp` was not used on purpose: with explicit
`oauthScopes` it would need `script.external_request` and another prompt.
